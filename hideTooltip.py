...
`search` function indirectly via `import re`.
    ...
    if not re.search("{{(.*:)*cloze:", self.note_type()["tmpls"][0]["qfmt"]):
        if self.addMode:
            tooltip(
...
from .modelFinder import getBasicNoteTypeList, modelExists
...
def _onClozeNew(self, *, _old):
    ...
    currentModelName = self.note.note_type()["name"]
...
def hookReSearch():
    ...
    # Hook this template
    # if not re.search("{{(.*:)*cloze:", self.note.note_type()["tmpls"][0]["qfmt"]):
